...
if [ $RDK_PLATFORM_SOC = "intel" ]; then
    ...
    if [ !{$OEM_VENDOR} = "BRCM" ]; then 
       ...
       export CC="${GCC_PREFIX}-gcc --sysroot ${_TMSYSROOT} -I${_TMTGTBUILDROOT}/comps/generic_apps/usr/include -I${_TMTGTBUILDROOT}/comps/generic_apps/usr/include/directfb -I${_TMTGTBUILDROOT}/comps/generic_apps/usr/include/fusiondale"
       export CXX="${GCC_PREFIX}-g++ --sysroot ${_TMSYSROOT} -I${_TMTGTBUILDROOT}/comps/generic_apps/usr/include -I${_TMTGTBUILDROOT}/comps/generic_apps/usr/include/directfb -I${_TMTGTBUILDROOT}/comps/generic_apps/usr/include/fusiondale"
	  ...
	  #export COMBINED_ROOT=$COMBINED_ROOT..
	  ...
	  export FUSION_PATH="${FSROOT}/usr"
	  export LDFLAGS+="-L${FSROOT}/vendor/lib \
	  -L${FSROOT}/usr/lib \
	  -L${IARM_PATH}/install \
	  -L${FSROOT}/vendor/lib/utopia \
	  -lIARMBus \
	  -ldirect -lfusiondale -lfusion -lpthread -ldirectfb -lglib-2.0 -lz -llinux"
	  export CFLAGS+="-I${FSROOT}/usr/include/utopia"
   elif [ $RDK_PLATFORM_SOC = "intel-x86-pc" ]; then
        export CC=gcc
        export CXX=g++
        export OPENSOURCE_BASE=$BUILDS_DIR/opensource
        export DFB_LIB=$RDK_FSROOT_PATH/usr/local/lib
        export IARM_PATH=$BUILDS_DIR/iarmbus
        export FUSION_PATH=$OPENSOURCE_BASE/src/FusionDale
        export GLIB_INCLUDE_PATH=$RDK_FSROOT_PATH/usr/local/include/glib-2.0/
        export GLIB_LIBRARY_PATH=$RDK_FSROOT_PATH/usr/local/lib/
        export GLIB_CONFIG_INCLUDE_PATH=$GLIB_LIBRARY_PATH/glib-2.0/
        export GLIBS='-lglib-2.0 -lz'	
    fi
...
function install()
{
	...
	if [ $RDK_PLATFORM_SOC = "intel" ]; then
		...
		if [ "$BUILD_CONFIG" != "rdkemulator" ]; then	
       			sudo cp -v ${RDK_SOURCE_PATH}/generic/ir/include/*.h ${RDK_FSROOT_PATH}/usr/local/include/
		fi
	fi
